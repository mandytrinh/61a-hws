...
def shuffle(cards):
    ...
    for index in range(0,(len(cards)//2)):
      combined_list.append(first_half[index])
      combined_list.append(second_half[index])
...
def has_prefix(s, prefix):
    ...
    def flattens_list(linked_list):
        ...
        return reg_list
    flattened_s =flattens_list(s)
    flattened_prefix = flattens_list(prefix)
    for index in range(0,len(flattened_prefix)):
        if flattened_s[index] != flattened_prefix[index]:
            return False
    return True
x = link(3, link(4, link(6, link(6, empty))))
